...
function wireLinks() {
  ...
  wireExternalLink($("#linkLinkedinContact"), LINKS.LINKEDIN_URL);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(LINKS.CONTACT_FORM_ENDPOINT, {
        ...
        // Web3Forms erwartet den Access-Key im Payload; er bestimmt das Empfängerpostfach.
        body: JSON.stringify({ access_key: LINKS.CONTACT_FORM_ACCESS_KEY, ...data }),
      });
      if (!response.ok) throw new Error("Serverfehler");
      showStatus("success", t("contact.statusSuccess"));
      form.reset();
    } catch (err) {
      showStatus("error", t("contact.statusError"));
    }
  });
})();
